...
function niinConfig(id, rows, item) {
  ...
  return {
    ...
    intro: `National Item Identification Number ${dashed} identifies ${item.toLowerCase()} items in the federal supply system. The part numbers and approved manufacturers mapped to this NIIN are listed below. Every line is quoted in Factory New, New Surplus or Overhauled condition, stated explicitly.`,
    linkList: {
      title: "Approved Manufacturers",
      items: mfrs.map((m) => [m, `/nsn/manufacturer/${slugify(m)}/`]),
    },
...
function cageConfig(id, rows, seed) {
  ...
  return {
    ...
    intro: `Commercial and Government Entity code ${id.toUpperCase()} is assigned to ${mfr}. The registration details and the part numbers we hold against this entity are below. Submit an RFQ on any line for pricing, availability and condition in writing.`,
