...
/* ------------------------------------------------- quote-cart / part lookups */
//
// The two adapters the cart's draft row needs from the data layer, plus the
// roster it offers before a part number has been typed. Both are safe to call
// with "" and both are deterministic per part number, like every other reader
// in this file — the same typed number always offers the same sources.
//
// SWAP POINT: when the live parts feed lands, these two are the only functions
// the cart touches. Point them at the real index and the draft row is unchanged.

/** Every manufacturer we can quote — the blank-part-number fallback list. */
export const manufacturerRoster = MFR_POOL;

/** Known sources for a part, most-likely first. Empty for a blank number. */
export function manufacturersForPart(partNumber = "") {
  const pn = String(partNumber || "").trim();
  if (!pn) return [];
  const r = makeRandom(`partmfr:${pn.toUpperCase()}`);
  return r.sample(MFR_POOL, r.int(3, 6));
}

/**
 * Part lookup, used by the cart only to autofill a line's description on
 * commit. An unrecognised number is not an error — it's a sourcing request —
 * so this always answers for a non-blank query rather than returning nothing.
 */
export function searchParts(query = "", limit = 5) {
  const pn = String(query || "").trim();
  if (!pn || limit < 1) return [];
  const r = makeRandom(`partsearch:${pn.toUpperCase()}`);
  return [
    {
      partNumber: pn,
      manufacturer: manufacturersForPart(pn)[0] || "",
      description: r.pick(NSN_ITEM_POOL),
    },
  ].slice(0, limit);
}

export { MFR_POOL, CONDITIONS, PART_TYPE_POOL, NSN_ITEM_POOL };
