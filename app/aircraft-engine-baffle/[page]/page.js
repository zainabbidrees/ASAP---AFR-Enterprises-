...
// Legacy pagination URLs (/aircraft-engine-baffle/page-{N}/). The listing data is a fixed shell, so
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: { canonical: "/aircraft-engine-baffle/" },
...
export default function PaginatedPage({ params }) {
  const m = PAGE_RE.exec(params.page);
  if (!m) notFound();
  if (m[1] === "1") redirect("/aircraft-engine-baffle/");
  return <Page1 />;
}
