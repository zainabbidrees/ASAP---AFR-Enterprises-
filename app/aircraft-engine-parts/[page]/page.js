...
// Legacy pagination URLs (/aircraft-engine-parts/page-{N}/). The listing data is a fixed shell, so
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: { canonical: "/aircraft-engine-parts/" },
...
export default function PaginatedPage({ params }) {
  const m = PAGE_RE.exec(params.page);
  if (!m) notFound();
  if (m[1] === "1") redirect("/aircraft-engine-parts/");
  return <Page1 />;
}
